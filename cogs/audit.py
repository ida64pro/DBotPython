...
class AUDIT(commands.Cog):
    ...
    async def on_typing(self, channel, user, when):
        ...
        with open(r'logs_user_typing.txt', 'a', encoding='utf-8') as file:
            file.write(
                f'LOG SERVER USER TYPING - {datetime.datetime.now()}\nUSER: {user} ~ {user.id}  |  CHANNEL: {channel} ~ {channel.id}\n\n')
    ...
    async def on_guild_update(self, before, after):
        ...
        if before.name != after.name:
            embed = disnake.Embed(title=f"Обновление сервера < Название >.", description=f"", color=color)
        ...
        if before.afk_channel != after.afk_channel:
            embed = disnake.Embed(title=f"Обновление сервера < АФК канал >.", description=f"", color=color)
        ...
        if before.afk_timeout != after.afk_timeout:
            embed = disnake.Embed(title=f"Обновление сервера < AFK Timeout >.", description=f"", color=color)
        ...
        if before.icon != after.icon:
            embed = disnake.Embed(title=f"Обновление сервера < Иконка >.", description=f"", color=color)
        ...
        if before.owner != after.owner:
            embed = disnake.Embed(title=f"Обновление сервера < Создатель >.", description=f"", color=color)
        ...
        if before.system_channel != after.system_channel:
            embed = disnake.Embed(title=f"Обновление сервера < Системный канал >.", description=f"", color=color)
        ...
        if before.rules_channel != after.rules_channel:
            embed = disnake.Embed(title=f"Обновление сервера < Канал правил >.", description=f"", color=color)
        ...
        if before.public_updates_channel != after.public_updates_channel:
            embed = disnake.Embed(title=f"Обновление сервера < Канал объявлений >.", description=f"", color=color)
        ...
        if before.preferred_locale != after.preferred_locale:
            embed = disnake.Embed(title=f"Обновление сервера < Язык >.", description=f"", color=color)
        ...
        if before.explicit_content_filter != after.explicit_content_filter:
            embed = disnake.Embed(title=f"Обновление сервера < Фильтр контента >.", description=f"", color=color)
        ...
        if before.verification_level != after.verification_level:
            embed = disnake.Embed(title=f"Обновление сервера < Уровень верефикации >.", description=f"", color=color)
        ...
        if before.default_notifications != after.default_notifications:
            embed = disnake.Embed(title=f"Обновление сервера < Уведомления >.", description=f"", color=color)
        ...
        if before.description != after.description:
            embed = disnake.Embed(title=f"Обновление сервера < Описание сервера >.", description=f"", color=color)
        ...
        if before.splash != after.splash:
            embed = disnake.Embed(title=f"Обновление сервера < Заставка сервера >.", description=f"", color=color)
    ...
    async def on_application_command_create(self, command):
        with open(r'logs_commands_register.txt', 'a', encoding='utf-8') as file:
            file.write(
                f'LOG SERVER COMMAND < CREAT > - {datetime.datetime.now()}\nCOMMAND: {command}  |  NAME: {command.name}  |  ID: {command.id}\n\n')
    ...
    async def on_application_command_delete(self, command):
        with open(r'logs_commands_register.txt', 'a', encoding='utf-8') as file:
            file.write(
                f'LOG SERVER COMMAND < DELETE > - {datetime.datetime.now()}\nCOMMAND: {command}  |  NAME: {command.name}  |  ID:{command.id}\n\n')

    @commands.Cog.listener()
    async def on_guild_channel_create(self, channel):
        creator = await get_channel_creator(channel.guild, channel)
        embed = disnake.Embed(title=f"Канал создан.", description=f"", color=color)
    ...
    async def on_guild_channel_update(self, before, after):
        ...
        embed = disnake.Embed(title=f"Канал изменён.", description=f"", color=color)
    ...
    async def on_guild_channel_delete(self, channel):
        ...
        embed = disnake.Embed(title=f"Канал удалён.", description=f"", color=color)
        embed.add_field(name=f"Канал", value=f"{channel.mention}", inline=True)
        embed.add_field(name=f"Имя канала", value=f"{channel.name}", inline=True)
        embed.add_field(name=f"ID Канала", value=f"{channel.id}", inline=True)
    ...
    async def on_guild_channel_pins_update(self, channel, last_pin):
        ...
        embed = disnake.Embed(title=f"Закреплённые сообщения обновлены.", description=f"", color=color)
        embed.add_field(name=f"Канал", value=f"{channel.mention}", inline=True)
        embed.add_field(name=f"Имя канала", value=f"{channel.name}", inline=True)
        embed.add_field(name=f"ID Канала", value=f"{channel.id}", inline=True)
        if user_who_pinned:
    ...
    async def on_guild_role_create(self, role):
        ...
        embed = disnake.Embed(title=f"Роль создана.", description=f"", color=color)
    ...
    async def on_guild_role_update(self, before, after):
        ...
        embed = disnake.Embed(title=f"Роль изменена.", description=f"", color=color)
    ...
    async def on_guild_role_delete(self, role):
        ...
        embed = disnake.Embed(title=f"Роль удалена.", description=f"", color=color)
        embed.add_field(name=f"Имя роли", value=f"{role.name}", inline=True)
        embed.add_field(name=f"Цвет роли", value=f"{role.color}", inline=True)
        embed.add_field(name=f"ID роли", value=f"{role.id}", inline=True)
        embed.add_field(name=f"Удалил", value=f"{deleter}", inline=True)
    ...
    async def on_thread_create(self, thread):
        embed = disnake.Embed(title=f"Ветка создана.", url=f"", description=f"", color=color)
    ...
    async def on_thread_update(self, before, after):
        ...
        embed = disnake.Embed(title=f"Ветка изменена.", url=f"", description=f"", color=color)
    ...
    async def on_thread_delete(self, thread):
        ...
        embed = disnake.Embed(title=f"Ветка удалена.", url=f"", description=f"", color=color)
    ...
    async def on_guild_emojis_update(self, guild, before, after):
        ...
        if added_emojis:
            for emoji in added_emojis:
                embed = disnake.Embed(title=f"Эмодзи добавлено.", url=f"", description=f"", color=color)
        ...
        for old_emoji in before:
            for new_emoji in after:
                if old_emoji.id == new_emoji.id and old_emoji.name != new_emoji.name:
                    embed = disnake.Embed(title=f"Эмодзи изменено.", url=f"", description=f"", color=color)
        ...
        if removed_emojis:
            for emoji in removed_emojis:
                embed = disnake.Embed(title=f"Эмодзи удалено.", url=f"", description=f"", color=color)
                embed.add_field(name=f"Эмодзи", value=f'<:{emoji.name}:{emoji.id}>', inline=True)
                embed.add_field(name=f"Имя", value=f'{emoji.name}', inline=True)
                embed.add_field(name=f"ID", value=f'{emoji.id}', inline=True)
                embed.timestamp = datetime.datetime.now()
    ...
    async def on_member_ban(self, guild, user):
        ...
        embed = disnake.Embed(title=f"Участник забанен.", url=f"", description=f"", color=color)
    ...
    async def on_member_unban(self, guild, user):
        ...
        embed = disnake.Embed(title=f"Участник разбанен.", url=f"", description=f"", color=color)
        embed.add_field(name=f"Участник", value=f'{user}', inline=True)
        embed.add_field(name=f"Ник", value=f'{user.name}', inline=True)
        embed.add_field(name=f"ID", value=f'{user.id}', inline=True)
        embed.add_field(name=f"Админ", value=f'{admin}', inline=True)
    ...
    async def on_member_mute(self, guild, user):
        audit = self.bot.get_channel(LOG_AUDIT)
        admin = await get_mute_admin(guild, user)
        embed = disnake.Embed(title=f"Участник замьючен.", url=f"", description=f"", color=color)
        embed.add_field(name=f"Участник", value=f'{user}', inline=True)
        embed.add_field(name=f"Ник", value=f'{user.name}', inline=True)
        embed.add_field(name=f"ID", value=f'{user.id}', inline=True)
        embed.add_field(name=f"Админ", value=f'{admin}', inline=True)
    ...
    async def on_invite_create(self, invite):
        audit = self.bot.get_channel(LOG_AUDIT)
        embed = disnake.Embed(title=f"Приглашение создано.", url=f"", description=f"", color=color)
    ...
    async def on_invite_delete(self, invite):
        ...
        embed = disnake.Embed(title=f"Приглашение Удалено.", url=f"", description=f"", color=color)
        if deleter:
            embed.add_field(name=f"Удалил", value=f'{deleter}', inline=True)
        else:
    ...
    async def on_reaction_add(self, reaction, user):
        audit = self.bot.get_channel(LOG_AUDIT)
        embed = disnake.Embed(title=f"Реакция добавлена.", url=f"", description=f"", color=color)
        embed.add_field(name=f"Участник", value=f'{user}', inline=True)
    ...
    async def on_reaction_remove(self, reaction, user):
        audit = self.bot.get_channel(LOG_AUDIT)
        embed = disnake.Embed(title=f"Реакция удалена.", url=f"", description=f"", color=color)
        embed.add_field(name=f"Участник", value=f'{user}', inline=True)
        embed.add_field(name=f"Реакция", value=f'{reaction.emoji}', inline=True)
        embed.add_field(name=f"ID сообщения", value=f'{reaction.message.id}', inline=True)
        embed.timestamp = datetime.datetime.now()
    ...
    async def on_webhooks_update(self, channel):
        ...
        if webhook_create:
            user = webhook_create.user
            audit = self.bot.get_channel(LOG_AUDIT)
            embed = disnake.Embed(title=f"Вебхук создан.", url=f"", description=f"", color=color)
            ...
            audit = self.bot.get_channel(LOG_AUDIT)
            embed = disnake.Embed(title=f"Вебхук изменён.", url=f"", description=f"", color=color)
            ...
            audit = self.bot.get_channel(LOG_AUDIT)
            embed = disnake.Embed(title=f"Вебхук удалён.", url=f"", description=f"", color=color)
    ...
    async def on_message_delete(self, message):
        if message.author.id == self.bot.user.id:  # self.
            return

        deleter = await get_message_deleter(message.guild, message)
        embed = disnake.Embed(title=f"Сообщение участника было удалено.", description=f"", color=color)
    ...
    async def on_message_edit(self, message_before, message_after):
        if message_before.author.id == self.bot.user.id:  # self.
            return
        if message_after.author.id == self.bot.user.id:  # self.
            return

        embed = disnake.Embed(title=f"Сообщение участника было отредактировано.", description=f"", color=color)
    ...
    async def on_member_update(self, before, after):
        if before.roles != after.roles:
            emb = disnake.Embed(title=f'Обновление ролей участника.', colour=color)
            async for event in before.guild.audit_logs(limit=1, action=disnake.AuditLogAction.member_role_update):
                ...
                emb.add_field(name="Изменённые роли:", value=", ".join(
                    [getattr(r, "mention", r.id) for r in event.before.roles or event.after.roles]), inline=False)
        ...
        if before.nick != after.nick:
            emb = disnake.Embed(title=f'Обновление ника участника на сервере.', colour=color)
            emb.add_field(name='Участник:', value=f"{before.mention}", inline=True)
            emb.add_field(name='Ник до:', value=f"{before.nick}", inline=True)
            emb.add_field(name='Ник после:', value=f"{after.nick}", inline=True)
            channel = self.bot.get_channel(LOG_AUDIT)
            await channel.send(embed=emb)

        if before.status != after.status:
            emb = disnake.Embed(title=f'Обновление статуса участника на сервере.', colour=color)
        ...
        if before.activities != after.activities:
            emb = disnake.Embed(title=f'Обновление активности участника на сервере.', colour=color)
            emb.add_field(name='Юзер:', value=f"{after.mention}", inline=True)
    ...
    async def on_user_update(self, before, after):
        if before.name != after.name:
            emb = disnake.Embed(title=f'Обновление ника юзера.', colour=color)
            emb.add_field(name='Юзер:', value=f"{after.mention}", inline=True)
        ...
        if before.discriminator != after.discriminator:
            emb = disnake.Embed(title=f'Обновление дискриминатора юзера.', colour=color)
            emb.add_field(name='Юзер:', value=f"{after.mention}", inline=True)
        ...
        if before.avatar != after.avatar:
            emb = disnake.Embed(title=f'Обновление аватарки юзера.', colour=color)
            emb.add_field(name='Юзер:', value=f"{after.mention}", inline=True)
    ...
    async def on_thread_member_join(self, member: disnake.Member = None):
        audit = self.bot.get_channel(LOG_AUDIT)
        embed = disnake.Embed(title=f"Участник зашел в ветку.", url=f"", description=f"", color=color)
    ...
    async def on_thread_member_remove(self, member: disnake.Member = None):
        audit = self.bot.get_channel(LOG_AUDIT)
        embed = disnake.Embed(title=f"Участник вышел из ветки.", url=f"", description=f"", color=color)
        embed.add_field(name=f"Участник", value=f'<@{member.id}>', inline=True)
        embed.add_field(name=f"ID участника", value=f'{member.id}', inline=True)
        embed.add_field(name=f"Ветка", value=f'<#{member.thread_id}>', inline=True)
        embed.timestamp = datetime.datetime.now()
    ...
    async def on_member_join(self, member: disnake.Member = None, guild: disnake.Guild = None):
        embed = disnake.Embed(title=f"Участник присоеденился к серверу.", url=f"", description=f"", color=color)
        ...
        embed.add_field(name=f"ID участника", value=f'{member.id}', inline=True)
        embed.add_field(name=f"Дата регистрации:",
                        value=f'{disnake.utils.format_dt(member.created_at, "f")} ({disnake.utils.format_dt(member.created_at, "R")})',
                        inline=True)
        embed.add_field(name=f"Присоеденился:",
                        value=f'{disnake.utils.format_dt(member.joined_at, "f")} ({disnake.utils.format_dt(member.joined_at, "R")})',
                        inline=True)
        embed.timestamp = datetime.datetime.now()
        audit = self.bot.get_channel(LOG_AUDIT)
        await audit.send(embed=embed)
    ...
    async def on_member_remove(self, member: disnake.Member = None, guild: disnake.Guild = None):
        embed = disnake.Embed(title=f"Участник вышел с сервера.", url=f"", description=f"", color=color)
        embed.set_thumbnail(url=member.avatar.url if member.avatar is not None else member.default_avatar)
        embed.add_field(name=f"Участник", value=f'{member.mention}', inline=True)
        embed.add_field(name=f"Ник", value=f'{member}', inline=True)
        embed.add_field(name=f"ID участника", value=f'{member.id}', inline=True)
    ...
    async def on_voice_state_update(self, member: disnake.Member, before: disnake.VoiceState,
        ...
        logs = self.bot.get_channel(LOG_AUDIT)  # self.
        if before.channel is None:
            embed = disnake.Embed(title=f"Участник зашёл в войс.", description=f"", timestamp=datetime.datetime.now(),
                                  color=color)
            ...
            embed = disnake.Embed(title=f"Участник вышел из войса.", description=f"", timestamp=datetime.datetime.now(),
                                  color=color)
            embed.add_field(name=f"Участник", value=f"{member}({member.mention})", inline=True)
            embed.add_field(name=f"ID участника", value=f"{member.id}", inline=True)
            ...
            embed = disnake.Embed(title=f"Участник перешёл.", description=f"", timestamp=datetime.datetime.now(),
                                  color=color)
            embed.add_field(name=f"Участник", value=f"{member}({member.mention})", inline=True)
            embed.add_field(name=f"ID участника", value=f"{member.id}", inline=True)
